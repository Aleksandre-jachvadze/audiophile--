  ...
  return (
    <div className="lg:basis-36 flex items-center justify-end">
      <Popover className="relative z-50">
        <PopoverButton aria-label="Open Shopping Cart">
        ...
        <PopoverPanel
          transition
          className="space-y-6 absolute right-0 max-sm:w-80 w-96 top-20 text-zinc-950 font-manrope ml-4 rounded-lg bg-white px-6 py-8 duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0"
        >
          {isEmpty ? (
            <div>
            ...
            <>
              <div className="flex justify-between">
                <span className="uppercase font-semibold tracking-wider">Cart ({cart.length})</span>
                <button
              ...
              <div className="flex justify-between">
                ...
                <span className="font-black">{formatCurrency(totalPurchase)}</span>
              </div>

              <CloseButton
                onClick={handleCheckout}
                className="uppercase w-full bg-custom-orange text-white py-3 tracking-wider text-sm"
              >
                Checkout
              </CloseButton>
            </>
